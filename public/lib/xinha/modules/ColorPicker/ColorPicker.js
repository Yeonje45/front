/**
  ...
  Xinha.colorPicker = function (params)
  {
    ...
    td.appendChild(document.createTextNode(Xinha._lc('색상선택', 'ColorPicker')));
    ...
    this.open = function(anchorage,element,initcolor)
    {
     ...
     // IE6 ONLY - prevent windowed elements (<SELECT>) to render above the colorpicker
      ...
      // 창의 너비를 20px 만큼 늘린다.
      this.table.style.width = (this.table.offsetWidth + 20 ) + 'px';
    };
...
//          unpickCell(valuerow,valuecol);
  ...
  //          unpickCell(pickrow,pickcol);
   ...
   //           this.style.borderColor = picker.saved_cells[this.thisrow][this.thiscol].colorCode;
        ...
        label.style.fontFamily = 'small-caption,caption,sans-serif';
        label.style.fontSize = 'x-small';
        label.appendChild(document.createTextNode(Xinha._lc('색상코드: ', 'ColorPicker')));
        ...
        okButton.appendChild(document.createTextNode(Xinha._lc('OK', 'ColorPicker')));
        okButton.style.border = '2px solid black'; // 테두리 추가
        okButton.style.display = 'block'; // 다음 줄로 이동
        okButton.style.marginTop = '10px'; // 약간 아래로 이동
        but.appendChild(okButton);
