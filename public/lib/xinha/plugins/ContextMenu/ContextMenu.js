...
ContextMenu.prototype.getContextMenu = function(target) {
	...
	if (!Xinha.is_gecko) {
		if (selection) {
			menu.push([ Xinha._lc("잘라내기", "ContextMenu"), function() { editor.execCommand("cut"); }, null, config.btnList["cut"][1] ],
				  [ Xinha._lc("복사하기", "ContextMenu"), function() { editor.execCommand("copy"); }, null, config.btnList["copy"][1] ]);
			menu.push([ Xinha._lc("붙여넣기", "ContextMenu"), function() { editor.execCommand("paste"); }, null, config.btnList["paste"][1] ]);
		}
	}
	...
	if(!menu.length)
  {
    if(selection)
    {
      menu.push([ Xinha._lc("잘라내기", "ContextMenu"), function() { alert(Xinha._lc("Please press CTRL-X (Command-X Mac) to cut")); }, null,  config.btnList["cut"][1] ]);
      menu.push([ Xinha._lc("복사하기", "ContextMenu"), function() { alert(Xinha._lc("Please press CTRL-C (Command-C Mac) to copy")); }, null,  config.btnList["copy"][1] ]);
    }
    menu.push([ Xinha._lc("붙여넣기", "ContextMenu"), function() { alert(Xinha._lc("Please press CTRL-V (Command-V Mac) to paste")); }, null,  config.btnList["paste"][1] ]);
  }
	...
	for (; target; target = target.parentNode) {
		...
		switch (tag) {
			...
			link = target;
			elmenus.push(null,
				     [ Xinha._lc("링크 수정", "ContextMenu"),
               function() { editor.config.btnList['createlink'][3](editor); },
				       Xinha._lc("Current URL is", "ContextMenu") + ': ' + link.href,
				       config.btnList["fixlink"][1] ],

				     [ Xinha._lc("링크 확인", "ContextMenu"),
				       function() { window.open(link.href); },
				       Xinha._lc("Opens this link in a new window", "ContextMenu")  + ': ' + link.href,
					   config.btnList["checklink"][1] ],

				     [ Xinha._lc("링크 삭제", "ContextMenu"),
				       function() {
					       if (confirm(Xinha._lc("이 링크를 정말 삭제할까요 ?", "ContextMenu") + "\n" +
							   Xinha._lc("링크주소 :", "ContextMenu") + " " + link.href)) {
						       while (link.firstChild)
							       link.parentNode.insertBefore(link.firstChild, link);
						       link.parentNode.removeChild(link);
					       }
				       },
				       Xinha._lc("Unlink the current element", "ContextMenu") + ': ' + link.href,
					   config.btnList["unlink"][1] ],
				);
			break;
        case "th":
		    case "td":
			...
			if( check == false )
			{
				...
				if(typeof config.btnList["TO-cell-prop"] != 'undefined' && check == false)
				{
								elmenus.push([ Xinha._lc("선택 셀 속성", "ContextMenu"),
								function() { tableOperation("TO-cell-prop"); },
								Xinha._lc("선택 셀 속성", "ContextMenu"),
				...
				if(typeof config.btnList["TO-cell-merge"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("셀 병합", "ContextMenu"),
								function() { tableOperation("TO-cell-merge"); },
								Xinha._lc("셀 병합", "ContextMenu"),
				...
				if(typeof config.btnList["TO-row-insert-above"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("선택 셀 위에 행 추가", "ContextMenu"),
								function() { tableOperation("TO-row-insert-above"); },
								Xinha._lc("선택 셀 위에 행 추가", "ContextMenu"),
								config.btnList["TO-row-insert-above"][1] ]);
				}
				
				if(typeof config.btnList["TO-row-insert-under"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("선택 셀 아래에 행 추가", "ContextMenu"),
								function() { tableOperation("TO-row-insert-under"); },
								Xinha._lc("선택 셀 아래에 행 추가", "ContextMenu"),
								config.btnList["TO-row-insert-under"][1] ]);
				}
				
				if(typeof config.btnList["TO-row-delete"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("선택 행 삭제", "ContextMenu"),
								function() { tableOperation("TO-row-delete"); },
								Xinha._lc("선택 셀 삭제", "ContextMenu"),
			...
			if (!tbo) break;
				...
				if(typeof config.btnList["TO-table-prop"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("테이블 속성", "ContextMenu"),
								function() { tableOperation("TO-table-prop"); },
								Xinha._lc("테이블 속성", "ContextMenu"),
								config.btnList["TO-table-prop"][1] ]);
				}
				
				if(typeof config.btnList["TO-col-insert-before"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("선택 셀 왼쪽에 열 추가", "ContextMenu"),
								function() { tableOperation("TO-col-insert-before"); },
								Xinha._lc("선택 셀 왼쪽 열 추가", "ContextMenu"),
								config.btnList["TO-col-insert-before"][1] ]);
				}
				
				if(typeof config.btnList["TO-col-insert-after"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("선택 셀 오른쪽에 열 추가", "ContextMenu"),
								function() { tableOperation("TO-col-insert-after"); },
								Xinha._lc("선택 셀 오른쪽 열 추가", "ContextMenu"),
								config.btnList["TO-col-insert-after"][1] ]);
				}
				
				if(typeof config.btnList["TO-col-delete"] != 'undefined' && check == false)
				{
						elmenus.push([ Xinha._lc("선택 열 삭제", "ContextMenu"),
								function() { tableOperation("TO-col-delete"); },
								Xinha._lc("선택 열 삭제", "ContextMenu"),
	...
	if (selection && !link && !img && !editor.getSelectedHTML().replace(/<[^>]+>/, '').match(/\s/))
  {
	...
	if (selection && !link)
		menu.push(null, [ Xinha._lc("링크 넣기", "ContextMenu"),
           function() { editor.config.btnList['createlink'][3](editor); },
