...
Xinha.Config.prototype.TableOperations = {
  ...
  //'renameSplitCellButton': '셀 병합 취소', // Split cell isn't a very obvious term, it implies being able to make new cells, really it is unmerging merged cells and can only be used in that context
...
function TableOperations(editor) {
  ...
  for (var i = 0; i < bl.length; ++i) {
    ...
    else if(typeof btn == 'string')
    {
      ...
      var id = "TO-" + btn[0];
      cfg.registerButton(id, Xinha._lc(btn[2], "TableOperations"), editor.imgURL(btn[0] + ".svg", "TableOperations"), false,
                         function(editor, id) {
...
TableOperations.prototype.buttonPress = function(editor, button_id) {
  ...
  case "TO-col-insert-after":   // 25년01월06일 테이블 행열 추가 수정
    var td = this.getClosestMatch(/^(td|th)$/i);
    if (!td) {
        break;
    }
    ...
    // 기존 <td>에서 스타일을 추출합니다.
    var tdStyle = td.getAttribute('style') || ''; // 기존 <td>의 스타일 속성 가져오기
  
    for (var i = rows.length; --i >= 0;) {
        var tr = rows[i];
        var otd = editor._doc.createElement("td");
          
        // 기존 <td>의 스타일을 새로 생성된 <td>에 적용합니다.
        otd.setAttribute('style', tdStyle);
  
        // 필요시 innerHTML을 설정합니다. (여기서 "mozbr"은 필요에 따라 수정)
        otd.innerHTML = mozbr;
...
TableOperations.btnList = [
  ...
  ["table-prop",       "table", "테이블속성"],
  null, // separator

  // ROWS
  // ["row-prop",         "tr", "선택 행 속성"],
  ["row-insert-above", "tr", "선택 셀 위에 행 추가"],
  ["row-insert-under", "tr", "선택 셀 아래에 행 추가"],
  ["row-delete",       "tr", "선택 행 삭제"],
  ["row-split",        "td[rowSpan!=1]", "Split row"],
  null,

  // COLS
  ["col-insert-before", ["td","th"], "선택 셀 왼쪽에 열 추가"],
  ["col-insert-after",  ["td","th"], "선택 셀 오른쪽에 열 추가"],
  ["col-delete",        ["td","th"], "선택 열 삭제"],
  ["col-split",         ["td[colSpan!=1]","th[colSpan!=1]"], "Split column"],
  null,

  // CELLS
  ["cell-prop",          ["td","th"], "선택 셀 속성"],
  ["cell-insert-before", ["td","th"], "왼쪽 셀 추가"],
  ["cell-insert-after",  ["td","th"], "오른쪽 셀 추가"],
  ["cell-delete",        ["td","th"], "셀 삭제"],
  ["cell-merge",         "tr", "셀 병합"],
  ...
  function apply() {
    ...
    if(editor.config.TableOperations.noFrameRulesOptions)
    {
      ...
      for(var i = 0; i < applyTo.length; i++)
      {
        ...
        // Check if the 'td' element has a style attribute, if not, add it
        if (!applyTo[i].style) {
          applyTo[i].style = {};  // add a style attribute if missing
        }
...
TableOperations.prototype.dialogRowCellProperties = function(cell) {
  ...
  typeLabel.innerHTML = Xinha._lc('셀 타입:', 'TableOperations');
  var typeSelect = dialog.createElement('select', 'to_type_select');
  typeSelect.options[0] = new Option(Xinha._lc('변경하지 않음','TableOperations'));
  typeSelect.options[1] = new Option(Xinha._lc('기본 (td)','TableOperations'), 'td');
  typeSelect.options[2] = new Option(Xinha._lc('헤더 (th)','TableOperations'), 'th');
