...
import { CreateRequirementModel, CreateRequirement, CheckDuplicateReqNumber } from 'models/RequirementModel'
import { RootState } from 'app/store'

interface IProps {
	isOpen: boolean
	req_group_code: string
	handleCreateRequirementModalState: () => void
	req_classification_code: string
	getRequirementsHandler: () => void
	requirementState_group_name: string | null
}

const CreateRequirementModal = ({ isOpen, req_group_code, handleCreateRequirementModalState, getRequirementsHandler, req_classification_code, requirementState_group_name }: IProps) => {
	const project = useSelector((state: RootState) => state.project).project

	const [requirementInpState, setRequirementInpState] = React.useState<CreateRequirementModel>({
		req_group_code: req_group_code,
		req_assort_code : "", // 요구사항 분류 PK
		req_number: "",// 요구사항 식별자
		req_title: "", // 요구사항 제목
		req_content: "", // 요구사항 내용
		req_tvm_code: "", // 시험/검증방법
	})	
	...
	const submitCreateRequirement = async (): Promise<void> => {
		if (requirementState_group_name === "SW 요구사항" && requirementInpState.req_tvm_code === "") {
			Swal.fire({
				icon: 'error',
				title: '입력 오류',
				text: '모든 항목을 입력해주세요.'
		...
		const res= await CreateRequirement(requirementInpState, req_classification_code)
		if (!res.success) {
			Swal.fire({
				icon: 'error',
				title: '요구사항 추가 실패',
		...
		handleCreateRequirementModalState()
		return
	}

	return (
		<Modal isOpen={isOpen} size="lg">
			<Modal.Head>{requirementState_group_name ? requirementState_group_name : ''} {`>`} 요구사항 추가</Modal.Head>
			<Modal.Body>
				<div className="flex flex-col items-center gap-4">
					...
					{/* 시험 검증 방법 */}
					{ requirementState_group_name === "SW 요구사항" && <div className="flex flex-wrap items-center w-full gap-4">
						<div className="flex items-center w-full">
							<label htmlFor="req_tvm_code" className="w-1/3">
								시험/검증방법
							</label>
							<Input.Select 
								id="req_tvm_code" 
								name="req_tvm_code" 
								className="w-full" 
								value={requirementInpState.req_tvm_code} 
								onChange={changeRequirementInpStateHandler}
							>
								<Input.Option value=""></Input.Option>
			...
			<Modal.Footer>
				<Button variant="primary" disabled={!isDuplicate} onClick={submitCreateRequirement}>추가</Button>
				<Button variant="secondary" onClick={handleCreateRequirementModalState}>취소</Button>
			</Modal.Footer>
		</Modal>
	)
}
