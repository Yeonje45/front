...
import { UpdateDefectManagementLogModel, DefectManagementLogModel, GetMeetingPeopleList, IMeetingPeopleModel, } from 'models/QualityManagement';

interface EditModalProps {
	content: string;
	disabled?: boolean;
	defectData: DefectManagementLogModel | null;
	onUpdateHandler: (defect: UpdateDefectManagementLogModel) => void;
}

const EditModal = ({content, disabled, defectData, onUpdateHandler}: EditModalProps) => {
	const project_state = useSelector((state: RootState) => state.project);

	const [isOpen, setIsOpen] = React.useState<boolean>(false);

	const [defect, setDefect] = React.useState<UpdateDefectManagementLogModel>({
		issue_id: 0,
		issue_name: '',
		issue_source: '',
		issue_comment: '',
  ...
  const [userList, setUserList] = React.useState<IMeetingPeopleModel[]>([]);
	...
	React.useEffect(() => {
		if (isOpen && defectData) {
			setDefect(() => {
        return {
          ...defectData,
          issue_priority: defectData.issue_priority ? defectData.issue_priority : 0,
        }
			});
			return
		}
					...
					<div 
						...
						<div className="p-3 border border-gray-300 grid grid-cols-2 gap-1 rounded-md bg-gray-50">
							<div className="text-start">
								...
								<Input id="issue_name" onChange={handleOnChange} value={defect.issue_name} disabled/>
							</div>
							<div className="text-start">
                  ...
                  { userList.map((user, index) => (
                    <Input.Option key={index} value={user.user_id}>{user.user_name}</Input.Option>
							...
							<div className="text-start">
								...
								<Input type="date" id="issue_solution_date" onChange={handleOnChange} value={defect.issue_solution_date || ''}/>
							</div>
						</div>
